...
import { applicationService, pwdService, statusLogService } from '@/services';
...
const ApplicationProfile = () => {
  ...
  const onSubmit = async(e: any) => {
    ...
    const response = await axios.post(
      ...
      `http://localhost:8000/api/applications/${application.id}/decision/`,
      formData,
      {
        headers: {
  ...
  useEffect(() => {
    ...
    const loadApplication = async() => {
      const data = await pwdService.get(pwdId);
    ...
    const loadStatusLog = async() => {
        const data = await statusLogService.getAll(pwdId);
        setStatusLog(data);
        console.log(data);
    }
  ...
  return (
    <div className='p-5'>
      ...
      <div className='flex gap-4'>
          <Link to='/pwd' className='block text-blue-500 mb-6 w-fit'>
          ...
          <p className='font-semibold'>PWD Profile</p>
      </div>
      ...
      <div className='space-y-4 mb-12 relative'>
        <Container>
          <div className='flex gap-4 items-center'>
            ...
            <div className='flex-1'>
              ...
              <p className='text-sm mt-2'><span className='text-gray-400'>Issued PWD No:</span> {pwd.issued_pwd_id}</p>
              <p className='text-sm'>
                ...
                {/* <div>
                    <button type='button' className='btn btn-sm font-normal'>Status Logs</button>
                </div> */}
        ...
        <AccordionItem title={`Status Report (${statusLog.length})`} opened={false}>
          {statusLog.map((stat, index) => (
            <div className='border border-gray-300 px-3 py-2 rounded-sm space-y-4' key={index}>
                <p>{stat.message}</p>
                <p className='text-xs text-gray-500'>Date: {stat.created_at}</p>
            </div>
          ))}
        </AccordionItem>
